...
import { 
  ...
  Activity, Loader2, RefreshCcw, Trash2, PieChart, TrendingUp
} from 'lucide-react';
...
const OtherInput = ({ value, onChange, placeholder = "SPECIFY..." }) => (
  ...
  >
    <input 
      className="w-full p-4 bg-white border-2 border-[#E8EEE9] focus:border-[#B5935E] rounded-2xl text-[11px] font-black tracking-widest uppercase outline-none transition-all shadow-sm placeholder:text-gray-300"
...
const PetalArchiveOS = () => {
  ...
  // --- 3. DATA PERSISTENCE ---
  useEffect(() => {
    const saved = localStorage.getItem('petal_archive_v18');
  ...
  const stats = useMemo(() => {
    ...
    liveData.forEach(row => {
      ...
      locationRevenue[loc] = (locationRevenue[loc] || 0) + (Number(row.price) || 0);
  ...
  const addToBasket = () => {
    ...
    setCurrentItem({ category: '', series: '', style: '', metal: '', chain: '', shape: '', base: '', colourLetter: '', price: '', otherCategory: '', otherChain: '', otherShape: '', otherBase: '', otherColour: '' });
    setStep(1); 
  };
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          <motion.div key="setup" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-6 space-y-6">
            ...
            <div className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-4">
              <div><Label>Event Name</Label><input className="w-full p-4 bg-[#FDFBF7] rounded-xl uppercase text-xs font-bold outline-none" value={session.eventName} onChange={e => setSession({...session, eventName: e.target.value.toUpperCase()})} /></div>
              ...
              <button onClick={() => {localStorage.setItem('petal_archive_v18', JSON.stringify(session)); setStep(1);}} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-bold shadow-xl uppercase">Open Tracker</button>
            </div>
          </motion.div>
        )}
        ...
        {view === 'input' && step > 0 && (
          <motion.div key="input" className="space-y-6">
            <div className="bg-[#1B3022] p-4 rounded-2xl flex justify-between items-center text-white shadow-lg">
              <div><p className="text-[10px] font-black uppercase text-[#B5935E]">{session.location === 'Others' ? session.otherLocation : session.location}</p></div>
              {basket.length > 0 && <button onClick={() => setStep(3)} className="bg-[#B5935E] px-5 py-2 rounded-xl text-[10px] font-black uppercase">Checkout ({basket.length})</button>}
            </div>

            {step === 1 && (
              <div className="space-y-7">
                <section><Label>1. Main Item</Label><div className="grid grid-cols-4 gap-2">{['Necklace', 'Bracelet', 'Ring', 'Earring', 'Bangle', 'Charm', 'Others'].map(c => (<GridBtn key={c} label={c} active={currentItem.category === c} onClick={() => setCurrentItem({...currentItem, category: c})} />))}</div>
                {currentItem.category === 'Others' && <OtherInput value={currentItem.otherCategory} onChange={(v) => setCurrentItem({...currentItem, otherCategory: v})} />}</section>
                ...
                {currentItem.chain === 'Others' && <OtherInput value={currentItem.otherChain} onChange={(v) => setCurrentItem({...currentItem, otherChain: v})} />}</section>
                <section><Label>3. Earring or Ring Type</Label><div className="grid grid-cols-3 gap-2">{['Signet', 'Adjustable', 'Hoop', 'Hook', 'Stud', 'Dangle', 'None'].map(st => (<GridBtn key={st} label={st} active={currentItem.style === st} onClick={() => setCurrentItem({...currentItem, style: st})} />))}</div></section>
                <button onClick={() => setStep(2)} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-black uppercase">Next Details</button>
              </div>
            )}

            {step === 2 && (
              <div className="space-y-6">
                <section><Label>4. Shape</Label><div className="grid grid-cols-3 gap-2">{['Round', 'Oval', 'Rectangle', 'Heart', 'Others'].map(sh => (<GridBtn key={sh} label={sh} active={currentItem.shape === sh} onClick={() => setCurrentItem({...currentItem, shape: sh})} />))}</div>
                {currentItem.shape === 'Others' && <OtherInput value={currentItem.otherShape} onChange={(v) => setCurrentItem({...currentItem, otherShape: v})} />}</section>
                ...
                {currentItem.base === 'Others' && <OtherInput value={currentItem.otherBase} onChange={(v) => setCurrentItem({...currentItem, otherBase: v})} />}</section>
                <section><Label>8. Colour / Letter</Label><div className="grid grid-cols-3 gap-2">{['Red', 'Blue', 'Yellow', 'Purple', 'Pink', 'Multi', 'Others'].map(col => (<GridBtn key={col} label={col} active={currentItem.colourLetter === col} onClick={() => setCurrentItem({...currentItem, colourLetter: col})} />))}</div>
                {currentItem.colourLetter === 'Others' && <OtherInput value={currentItem.otherColour} onChange={(v) => setCurrentItem({...currentItem, otherColour: v})} />}</section>
                ...
                <div className="flex gap-4"><button onClick={() => setStep(1)} className="flex-1 py-4 text-gray-400 font-bold uppercase text-[10px]">Back</button><button onClick={addToBasket} className="flex-[2] bg-[#B5935E] text-white py-4 rounded-2xl font-black shadow-xl">ADD TO BASKET</button></div>
              </div>
            )}
        ...
        {view === 'dashboard' && (
          <motion.div key="dash" className="space-y-6">
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100 space-y-4">
              <Label>Race & Age Splits</Label>
              <div className="space-y-4">
                <div className="flex h-3 bg-gray-100 rounded-full overflow-hidden">{Object.entries(stats.raceCounts).map(([race, count], i) => (<div key={race} style={{ width: `${(count / (stats.totalPieces || 1)) * 100}%`, backgroundColor: i % 2 === 0 ? '#B5935E' : '#1B3022' }} />))}</div>
                <div className="flex h-3 bg-gray-100 rounded-full overflow-hidden">{Object.entries(stats.ageCounts).map(([age, count], i) => (<div key={age} style={{ width: `${(count / (stats.totalPieces || 1)) * 100}%`, opacity: 1 - (i * 0.15), backgroundColor: '#1B3022' }} />))}</div>
              </div>
            </section>
        ...
        {view === 'history' && (
          <motion.div key="bi" className="space-y-6">
            ...
            <section className="bg-[#1B3022] p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden">
               ...
               <div className="relative z-10 space-y-4"><Label><span className="text-[#B5935E]">Venue Efficiency</span></Label><h4 className="text-xl font-serif italic">{session.location}</h4><div className="flex justify-between border-t border-white/10 pt-4"><div><p className="text-[9px] opacity-40 uppercase font-black">Location Total</p><p className="text-lg font-serif">RM {stats.locationRevenue[session.location]?.toLocaleString() || 0}</p></div><div className="text-right"><p className="text-[9px] opacity-40 uppercase font-black">Impact</p><p className="text-lg font-serif text-[#B5935E]">{((stats.locationRevenue[session.location] / (stats.totalRevenue || 1)) * 100).toFixed(1)}%</p></div></div></div>
            </section>
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100"><Label>Seasonality</Label>
          ...
          <motion.div key="settings" className="space-y-6">
            ...
            <section className="bg-white p-2 rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="space-y-1">
                {[{c: "Necklaces", i: [{n: "Cable Chain", p: "95"}, {n: "Paperclip", p: "105"}]}, {c: "Others", i: [{n: "Rings", p: "89"}]}].map((group, i) => (
            ...
            <a href={MASTER_SHEET_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full p-6 bg-[#E8EEE9] rounded-[2.5rem] text-[10px] font-black uppercase text-[#1B3022]"><ExternalLink size={16}/> Open Database</a>
            <div className="grid grid-cols-2 gap-4">
               <button onClick={() => {localStorage.clear(); window.location.reload();}} className="bg-white text-gray-400 py-6 rounded-[2rem] font-black text-[9px] uppercase border border-gray-100 flex flex-col items-center gap-2 shadow-sm"><RefreshCcw size={14}/> Reset App</button>
               <button onClick={() => {localStorage.removeItem('petal_archive_v18'); setStep(0); setView('input');}} className="bg-red-50 text-red-400 py-6 rounded-[2rem] font-black text-[9px] uppercase border border-red-100 flex flex-col items-center gap-2 shadow-sm"><Trash2 size={14}/> End Session</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
