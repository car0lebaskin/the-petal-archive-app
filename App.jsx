...
const PetalArchiveOS = () => {
  ...
  const OtherInput = ({ value, onChange, placeholder = "SPECIFY..." }) => (
    <motion.input 
      ...
      className="w-full mt-2 p-3 bg-[#FDFBF7] border border-dashed border-[#B5935E] rounded-xl text-xs font-bold uppercase outline-none"
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#1B3022] font-sans p-4 max-w-md mx-auto pb-32 overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'input' && step > 0 && (
          <motion.div key="input" className="space-y-6">
            <div className="bg-[#1B3022] p-4 rounded-2xl flex justify-between items-center text-white">
              <div><p className="text-[10px] font-black uppercase text-[#B5935E]">{session.location === 'Others' ? session.otherLocation : session.location}</p><p className="text-[11px] font-serif italic">{basket.length} Items in Basket</p></div>
        ...
        {/* DASHBOARD VIEW (Remaining logic same) */}
        {view === 'dashboard' && (
        ...
        {/* COMMAND CENTER (Remaining logic same) */}
        {view === 'settings' && (
          <motion.div key="settings" className="space-y-6">
            ...
            <section className="bg-white p-2 rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="space-y-1">
                {[{c: "Necklaces", i: [{n: "Cable / Snake Chain", p: "95"}, {n: "M-Paperclip / Paperclip", p: "105"}]},
                  {c: "Bracelets", i: [{n: "Snake / Box Chain", p: "95"}]},
                  {c: "Bangles & Rings", i: [{n: "Rings", p: "89"}]}
                ].map((group, i) => (
            ...
            <a href={MASTER_SHEET_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full p-6 bg-[#E8EEE9] rounded-[2.5rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-sm text-[#1B3022]"><ExternalLink size={16}/> Open Master Database</a>
            <div className="grid grid-cols-2 gap-4">
