...
import { 
  ...
  Calendar, Users, CheckCircle2, Package, TrendingUp, Clock, History, LogOut, ShoppingBag, Trash2, LineChart, Layers, FileText, Database, Download, RefreshCw, ChevronDown
} from 'lucide-react';
...
const PetalArchiveOS = () => {
  ...
  // --- MOCK DATA ---
  const priceList = [
    { category: 'Necklaces', items: [{ name: 'Plain (STG/STU)', price: 'RM 114' }, { name: 'CZ / Alphabet', price: 'RM 124' }] },
    { category: 'Bracelets', items: [{ name: 'Standard', price: 'RM 99' }, { name: 'Alphabet / CZ', price: 'RM 109' }] }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#1B3022] font-sans p-5 max-w-md mx-auto overflow-x-hidden pb-32">
      ...
      {step === 0 && (
        <div className="pt-10 space-y-6">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
            <div><Label>Event Name</Label><input className="w-full p-4 bg-[#FDFBF7] rounded-2xl border-none outline-none ring-1 ring-gray-100 uppercase text-xs font-bold text-[#1B3022]" placeholder="TRX BAZAAR" value={session.eventName} onChange={e => setSession({...session, eventName: toCaps(e.target.value)})} /></div>
            <div><Label>Organiser</Label><input className="w-full p-4 bg-[#FDFBF7] rounded-2xl border-none outline-none ring-1 ring-gray-100 uppercase text-xs font-bold text-[#1B3022]" placeholder="CURATE" value={session.organiser} onChange={e => setSession({...session, organiser: toCaps(e.target.value)})} /></div>
            <div>
              ...
              {session.location === 'Others' && <input className="w-full p-3 bg-[#FDFBF7] rounded-xl border-none ring-1 ring-gray-100 text-xs font-bold uppercase mt-2 text-[#B5935E]" placeholder="SPECIFY LOCATION..." value={session.otherLocation} onChange={e => setSession({...session, otherLocation: toCaps(e.target.value)})} />}
            </div>
            ...
            <button onClick={() => setStep(1)} disabled={!session.location || !session.eventName} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-bold text-lg shadow-xl shadow-green-900/20">Open Session</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#1B3022] p-6 rounded-[2rem] text-white shadow-xl flex flex-col justify-between h-36">
              ...
              <div className="text-[10px] text-[#B5935E] font-black uppercase italic tracking-widest">Progress: 70%</div>
            </div>
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-50 shadow-sm">
            ...
            <div className="space-y-3">
              {[{n: 'Necklace', c: 8}, {n: 'Earrings', c: 5}].map((item, i) => (
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-50 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-3"><p className="text-[8px] font-black text-gray-400 border-b">AGE</p><StatBar label="20s" percentage={75} /><StatBar label="30s" percentage={20} /></div>
            </div>
          </section>
        </div>
      )}
        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
            ...
            <div className="flex items-end gap-1 h-24 pt-4">
              {[{m:'J',r:40,t:'CNY'},{m:'F',r:80,t:'VAL'},{m:'M',r:30},{m:'A',r:60,t:'MOTHER'}].map((d, i) => (
      ...
      {/* 4. SETTINGS & PRICE LIST */}
      {view === 'settings' && step > 0 && (
        <div className="space-y-6 pb-20 animate-in slide-in-from-bottom">
          <header className="text-center py-6"><h2 className="text-3xl font-serif italic">Settings</h2></header>
          <section className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-[#1B3022] p-6 flex items-center justify-between text-white"><span className="text-[10px] font-black uppercase tracking-widest">Price Directory</span></div>
